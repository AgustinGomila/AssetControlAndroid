...
class GetClientPackages {
    ...
    private suspend fun suspendFunction(): Boolean = withContext(Dispatchers.IO) {
        if (!Statics.isOnline()) {
            ...
            msg = Statics.AssetControl.getContext().getString(R.string.no_connection)
    ...
    private fun getPackages(): Boolean {
        ...
        try {
            ...
            msg = "${
                Statics.AssetControl.getContext().getString(R.string.exception_error)
            }: ${ex.message}"
            return false
        } finally {
    ...
    private fun getResponse(connection: HttpsURLConnection): ArrayList<JSONObject> {
        ...
        try {
            ...
            msg = if (result.size > 0) {
                Statics.AssetControl.getContext().getString(R.string.success_response)
            } else {
