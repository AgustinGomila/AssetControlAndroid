...
import com.dacosys.assetControl.data.dataBase.route.RouteProcessStatusContract.RouteProcessStatusEntry.Companion.DESCRIPTION
import com.dacosys.assetControl.data.dataBase.route.RouteProcessStatusContract.RouteProcessStatusEntry.Companion.ROUTE_PROCESS_STATUS_ID
import com.dacosys.assetControl.data.dataBase.route.RouteProcessStatusContract.RouteProcessStatusEntry.Companion.TABLE_NAME
import com.dacosys.assetControl.data.dataBase.route.RouteProcessStatusContract.getAllColumns
...
class RouteProcessStatusDbHelper {
    ...
    fun select(): ArrayList<RouteProcessStatus> {
    ...
    fun selectById(id: Long): RouteProcessStatus? {
    ...
    fun selectByDescription(description: String): ArrayList<RouteProcessStatus> {
    ...
    private fun fromCursor(c: Cursor?): ArrayList<RouteProcessStatus> {
        val result = ArrayList<RouteProcessStatus>()
        c.use {
            if (it != null) {
                while (it.moveToNext()) {
                    val id = it.getInt(it.getColumnIndexOrThrow(ROUTE_PROCESS_STATUS_ID))
                    val temp = RouteProcessStatus.getById(id)
                    if (temp != null)
                        result.add(temp)
                }
            }
    ...
    companion object {
        ...
        const val CREATE_TABLE = ("CREATE TABLE IF NOT EXISTS [" + TABLE_NAME + "]"
                + "( [" + ROUTE_PROCESS_STATUS_ID + "] INT ( 11 ) NOT NULL UNIQUE ,"
