...
import com.dacosys.assetControl.data.dataBase.barcode.BarcodeLabelTargetContract.BarcodeLabelTargetEntry.Companion.BARCODE_LABEL_TARGET_ID
import com.dacosys.assetControl.data.dataBase.barcode.BarcodeLabelTargetContract.BarcodeLabelTargetEntry.Companion.DESCRIPTION
import com.dacosys.assetControl.data.dataBase.barcode.BarcodeLabelTargetContract.BarcodeLabelTargetEntry.Companion.TABLE_NAME
import com.dacosys.assetControl.data.dataBase.barcode.BarcodeLabelTargetContract.getAllColumns
...
class BarcodeLabelTargetDbHelper {
    ...
    fun sync(): Boolean {
        ...
        try {
            ...
            val objArray = BarcodeLabelTarget.getAll()
            for (obj in objArray) {
                Log.i(
    ...
    fun delete(barcodeLabelTarget: BarcodeLabelTarget): Boolean {
        return deleteById(barcodeLabelTarget.id)
    }
    ...
    fun select(): ArrayList<BarcodeLabelTarget> {
    ...
    fun selectById(id: Long): BarcodeLabelTarget? {
    ...
    fun selectByDescription(description: String): ArrayList<BarcodeLabelTarget> {
    ...
    private fun fromCursor(c: Cursor?): ArrayList<BarcodeLabelTarget> {
        val result = ArrayList<BarcodeLabelTarget>()
        c.use {
            if (it != null) {
                while (it.moveToNext()) {
                    val id = it.getLong(it.getColumnIndexOrThrow(BARCODE_LABEL_TARGET_ID))
                    val temp = BarcodeLabelTarget.getById(id)
                    if (temp != null)
                        result.add(temp)
                }
            }
    ...
    companion object {
        ...
        const val CREATE_TABLE = ("CREATE TABLE IF NOT EXISTS [" + TABLE_NAME + "]"
                + "( [" + BARCODE_LABEL_TARGET_ID + "] INT ( 11 ) NOT NULL UNIQUE ,"
