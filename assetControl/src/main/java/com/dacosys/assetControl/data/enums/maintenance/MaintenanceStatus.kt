...
data class MaintenanceStatus(val id: Int, val description: String) : Parcelable {
    ...
    companion object CREATOR : Parcelable.Creator<MaintenanceStatus> {
        ...
        var unknown = MaintenanceStatus(
            0, getContext().getString(R.string.unknown)
        )
        var repair = MaintenanceStatus(
            1, getContext().getString(R.string.maintenance_status_repair)
        )
        var income = MaintenanceStatus(
            2, getContext().getString(R.string.maintenance_status_income)
        )
        var underDiagnosis = MaintenanceStatus(
            3, getContext().getString(R.string.maintenance_status_under_diagnosis)
        )
        var diagnosed = MaintenanceStatus(
            4, getContext().getString(R.string.maintenance_status_diagnosed)
        )
        var cost = MaintenanceStatus(
            5, getContext().getString(R.string.maintenance_status_cost)
        )
        var approvedCost = MaintenanceStatus(
            6, getContext().getString(R.string.maintenance_status_approved_cost)
        )
        var underRepair = MaintenanceStatus(
            7, getContext().getString(R.string.maintenance_status_under_repair)
        )
        var repaired = MaintenanceStatus(
            8, getContext().getString(R.string.maintenance_status_repaired)
        )
        var finished = MaintenanceStatus(
            9, getContext().getString(R.string.maintenance_status_finished)
        )
        var repairImposible = MaintenanceStatus(
            10, getContext().getString(R.string.maintenance_status_repair_imposible)
        )
