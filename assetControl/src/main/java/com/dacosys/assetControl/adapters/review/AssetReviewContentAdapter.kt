...
class AssetReviewContentAdapter :
    ArrayAdapter<AssetReviewContent>,
    Filterable {
    ...
    private var visibleStatus: ArrayList<AssetReviewContentStatus> = ArrayList()
    private var checkedIdArray: ArrayList<Long> = ArrayList()

    private var arContArray: ArrayList<AssetReviewContent> = ArrayList()
    private var suggestedList: ArrayList<AssetReviewContent> = ArrayList()

    constructor(
        activity: AppCompatActivity,
        resource: Int,
        assetReviewContArray: ArrayList<AssetReviewContent>,
        suggestedList: ArrayList<AssetReviewContent>,
        listView: ListView?,
        multiSelect: Boolean,
        ...
        this.arContArray = assetReviewContArray
     ...
     * refresh.
     * */
    private fun refreshItems(originalList: ArrayList<AssetReviewContent>) {
     ...
     */
    override fun add(arc: AssetReviewContent?) {
        if (arc != null) {
            add(arrayListOf(arc), true)
        }
    }

    fun add(arContArray: ArrayList<AssetReviewContent>, selectItem: Boolean) {
        val all = getAll()
        val arcAdded: ArrayList<AssetReviewContent> = ArrayList()
        ...
        activity.runOnUiThread {
            ...
            if (!arcAdded.any()) return@runOnUiThread

            reportAssetReviewAdded(arcAdded)
            ...
            for (arc in arcAdded) {
                if (arc.contentStatusId != AssetReviewContentStatus.notInReview.id &&
                    ...
                    setChecked(content = arc, isChecked = true)
                } else {
                    setChecked(content = arc, isChecked = false)
                }
            }
    ...
    fun updateAsset(asset: Asset, selectItem: Boolean = true) {
        for (i in 0 until count) {
            val t = (getItem(i) as AssetReviewContent)
     ...
     */
    fun updateContent(
        arc: AssetReviewContent,
        assetReviewContentStatusId: Int,
        assetStatusId: Int,
        selectItem: Boolean,
        ...
        for (i in 0 until count) {
            ...
            if (t == arc) {
                activity.runOnUiThread {
                    ...
                    t.contentStatusId = assetReviewContentStatusId
                }

                if (changeCheckedState) {
                    if (t.contentStatusId != AssetReviewContentStatus.notInReview.id &&
                        t.contentStatusId != AssetReviewContentStatus.unknown.id
                    ) {
                ...
                if (selectItem) forceSelectItem(arc)
     ...
     */
    override fun remove(assetReviewContent: AssetReviewContent?) {
        if (assetReviewContent != null) {
            remove(arrayListOf(assetReviewContent))
        }
    }

    /**
     * Función que propiamente elimina el contenido de la colección.
     * También se ocupa de eliminar el Id de la colección de Ids marcados (checked).
     */
    fun remove(assets: ArrayList<AssetReviewContent>) {
        if (assets.isEmpty()) return

        val all = getAll()
        lastSelectedPos = currentPos()

        val assetsRemoved: ArrayList<AssetReviewContent> = ArrayList()
        activity.runOnUiThread {
            for (w in assets) {
                if (all.contains(w)) {
                    assetsRemoved.add(w)
            ...
            reportAssetReviewRemoved(assetsRemoved)
     ...
     */
    private fun reportAssetReviewAdded(arContArray: ArrayList<AssetReviewContent>) {
        if (suspendReport) {
            return
        }
        if (arContArray.size <= 0) {
            return
        }

        var res = ""
        for (assetReviewContent in arContArray) {
            res += "${assetReviewContent.code}, "
        }
     ...
     */
    private fun reportAssetReviewRemoved(arContArray: ArrayList<AssetReviewContent>) {
        if (suspendReport) {
            return
        }
        if (arContArray.size <= 0) {
    ...
    private fun getIndex(assetReviewContent: AssetReviewContent): Int {
        for (i in 0 until count) {
            val t = (getItem(i) as AssetReviewContent)
            if (t == assetReviewContent) {
                return i
            }
        }
        return -1
    ...
    fun getAll(): ArrayList<AssetReviewContent> {
        val r: ArrayList<AssetReviewContent> = ArrayList()
        for (i in 0 until count) {
    ...
    fun setChecked(checkedItems: ArrayList<AssetReviewContent>) {
        checkedItems.clear()
        setChecked(checkedItems, true)
    }

    private var isFilling = false
    private fun setChecked(contents: ArrayList<AssetReviewContent>, isChecked: Boolean) {
        if (isFilling) return
        isFilling = true
     ...
     * de la colección de Ids marcados (checked).
     */
    ...
    override fun sort(comparator: Comparator<in AssetReviewContent>) {
        super.sort(customComparator)
    }

    private val customComparator =
        Comparator { o1: AssetReviewContent?, o2: AssetReviewContent? ->
            AssetReviewContentComparator().compareNullable(
                o1,
                o2
            )
        }
     ...
     * Cambia a verdadero estado marcado (checked).
     * Hace scroll suave hasta el ítem.
     */
    fun forceSelectItem(a: AssetReviewContent) {
    ...
    fun selectItem(
        ...
        var pos = -1
        if (arc != null) pos = getPosition(arc)
    ...
    fun currentArCont(): AssetReviewContent? {
    ...
    fun getItems(): ArrayList<AssetReviewContent> {
        val r: ArrayList<AssetReviewContent> = ArrayList()
        for (i in 0 until count) {
            val t = getItem(i) ?: continue
            r.add(t)
    ...
    fun isStatusVisible(arc: AssetReviewContent?): Boolean {
        return if (arc != null) {
            (visibleStatus.contains(AssetReviewContentStatus.getById(arc.contentStatusId)))
        } else false
    }

    fun getVisibleStatus(): ArrayList<AssetReviewContentStatus> {
        return visibleStatus
    }

    fun setVisibleStatus(status: ArrayList<AssetReviewContentStatus>) {
    ...
    val assetsMissed: Int
        get() {
            return getAll().count { it.contentStatusId == AssetReviewContentStatus.notInReview.id }
        }

    val assetsRevised: Int
        get() {
            return getAll().count {
        ...
        return object : Filter() {
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if (constraint != null) {
                    ...
                    if (filterString.isNotEmpty()) {
                        ...
                        for (i in 0 until arContArray.size) {
                            filterableItem = arContArray[i]
            ...
            override fun publishResults(
                ...
                if (results != null && results.count > 0) {
                    suggestedList.addAll(results.values as ArrayList<AssetReviewContent>)
                    notifyDataSetChanged()
                } else {
    ...
    fun isFilterable(filterableAsset: AssetReviewContent, filterString: String): Boolean =
    ...
    companion object {
        class AssetReviewContentComparator(private val priorityText: String) :
            Comparator<AssetReviewContent> {
            constructor() : this("")

            fun compareNullable(o1: AssetReviewContent?, o2: AssetReviewContent?): Int {
        ...
        fun sortItems(originalList: ArrayList<AssetReviewContent>): ArrayList<AssetReviewContent> {
            ...
            fun follow(assetReviewContent: AssetReviewContent): List<AssetReviewContent> {
                return listOf(assetReviewContent) + (groups[assetReviewContent.assetId]
            ...
            return originalList.map { it.parentId }
                ...
                .flatMap(::follow) as ArrayList<AssetReviewContent>
        }
    }
}