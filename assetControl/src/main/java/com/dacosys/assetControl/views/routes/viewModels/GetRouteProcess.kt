    ...
    private suspend fun suspendFunction(): RouteProcessResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (!allAttrCompIdAvailable.containsAll(allAttrCompIdRoute)) {
                ...
                return@withContext RouteProcessResult(routeProcess = null,
                    newProcess = false,
                    error = ErrorResult(AssetControlApp.getContext()
            ...
            if (rpArray.size <= 0) {
                ...
                return@withContext RouteProcessResult(routeProcess = routeProcess,
                    newProcess = false)
            }
        } finally {
            db.endTransaction()
