...
import com.dacosys.assetControl.model.assets.asset.`object`.AssetCRUD.Companion.RC_ERROR_OBJECT_NULL
import com.dacosys.assetControl.model.assets.asset.`object`.AssetCRUD.Companion.RC_ERROR_UPDATE
import com.dacosys.assetControl.model.assets.asset.`object`.AssetCRUD.Companion.RC_INSERT_OK
import com.dacosys.assetControl.model.assets.asset.`object`.AssetCRUD.Companion.RC_UPDATE_OK
...
class AssetCRUDActivity : AppCompatActivity(),
    ...
    private fun setBottomPanelAnimation() {
        ...
        binding.expandButton?.setOnClickListener {
            ...
            if (panelBottomIsExpanded) nextLayout.load(
                this,
                R.layout.asset_crud_activity_bottom_panel_collapsed
            )
    ...
    private fun changeCode(tempCode: String) {
        if (asset != null ||
            (assetCRUDFragment?.getDescription() ?: "").isNotEmpty() ||
            (assetCRUDFragment?.getCode() ?: "").isNotEmpty()
        ) {
            JotterListener.pauseReaderDevices(this)
            try {
