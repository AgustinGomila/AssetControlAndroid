...
import com.dacosys.assetControl.model.reviews.assetReview.dbHelper.AssetReviewAdapter
...
import com.dacosys.imageControl.dbHelper.DbCommands.Companion.selectByProgramObjectObj1Obj2
import org.parceler.Parcels
import java.io.File
import java.util.concurrent.ThreadLocalRandom

@Suppress("UNCHECKED_CAST")
class AssetReviewSelectActivity : AppCompatActivity(),
    ...
    private fun setPanels() {
        ...
        if (panelBottomIsExpanded) {
            ...
            currentLayout.load(
                this,
                R.layout.asset_review_select_bottom_panel_collapsed
            )
        }
    ...
    private fun assetReviewSelect() {
        if (arrayAdapter != null) {
            ...
            if (ar.statusId == AssetReviewStatus.onProcess.id) {
                ...
                makeText(
                    binding.root,
                    getString(R.string.selected_review_is_not_in_process),
                    ERROR
                )
            }
        }
        ...
        if (arrayAdapter != null) {
            ...
            try {
                val localImages = selectByProgramObjectObj1Obj2(
                    Table.assetReview.tableId.toString(),
                    ar.collectorAssetReviewId.toString(),
                    ""
                )
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            AssetReviewStatus.onProcess.id ->
                if (item.isChecked && !visibleStatus.contains(AssetReviewStatus.onProcess)) {
            ...
            AssetReviewStatus.completed.id ->
                if (item.isChecked && !visibleStatus.contains(AssetReviewStatus.completed)) {
            ...
            AssetReviewStatus.transferred.id ->
                if (item.isChecked && !visibleStatus.contains(AssetReviewStatus.transferred)) {
            ...
            AssetReviewStatus.unknown.id ->
                if (item.isChecked && !visibleStatus.contains(AssetReviewStatus.unknown)) {
