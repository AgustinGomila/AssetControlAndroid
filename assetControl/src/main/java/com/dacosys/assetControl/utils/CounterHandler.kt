...
class CounterHandler private constructor(builder: Builder) {
    ...
    private var startNumber: Long = 0
    private val incrementalView: View?
    private val decrementalView: View?
    private var minRange: Long = -1
    private var maxRange: Long = -1
    ...
    private fun increment() {
        ...
        if (maxRange != -1L) {
            if (number + counterStep <= maxRange) {
                ...
                number = if (minRange == -1L) 0 else minRange
            }
        } else {
            number += counterStep
        }
    ...
    private fun decrement() {
        ...
        if (minRange != -1L) {
            if (number - counterStep >= minRange) {
                ...
                number = if (maxRange == -1L) 0 else maxRange
            }
        } else {
            number -= counterStep
        }
    ...
    interface CounterListener {
        fun onIncrement(view: View?, number: Long)

        fun onDecrement(view: View?, number: Long)
    }

    class Builder {
        internal var incrementalView: View? = null
        internal var decrementalView: View? = null
        internal var minRange: Long = -1
        internal var maxRange: Long = -1
        internal var startNumber: Long = 0
        ...
        fun minRange(`val`: Long): Builder {
            minRange = `val`
            return this
        }

        fun maxRange(`val`: Long): Builder {
            maxRange = `val`
            return this
        }

        fun startNumber(`val`: Long): Builder {
            startNumber = `val`
            return this
        }
