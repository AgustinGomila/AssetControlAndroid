package com.example.assetControl.ui.activities.route
...
import com.example.assetControl.data.model.common.ExprResultIntString
import com.example.assetControl.data.model.common.Parameter
import com.example.assetControl.data.model.common.SaveRouteProcess
import com.example.assetControl.data.model.common.SkipAll
...
import com.example.assetControl.databinding.RouteProcessContentActivityBinding
...
import com.example.assetControl.ui.activities.dataCollection.DccActivity
import com.example.assetControl.ui.activities.dataCollection.DccDetailActivity
import com.example.assetControl.ui.activities.location.WarehouseAreaDetailActivity
...
import com.example.assetControl.ui.adapters.interfaces.Interfaces.UiEventListener
import com.example.assetControl.ui.adapters.route.RpcRecyclerAdapter
import com.example.assetControl.ui.adapters.route.RpcRecyclerAdapter.PAYLOADS
...
import com.example.assetControl.viewModel.route.RouteProcessResult
import com.example.assetControl.viewModel.route.SaveRouteProcessViewModel
...
class RouteProcessContentActivity : AppCompatActivity(), Scanner.ScannerListener,
    ...
    private fun setBottomPanelAnimation() {
        ...
        binding.expandButton?.setOnClickListener {
            ...
            else binding.expandButton?.text = context.getString(R.string.more_options)
        }
    }
    ...
    private fun fill(level: Int) {
        ...
        if (routeComposition?.any() != true) {
            ...
            val data = Intent()
            data.putExtra("error_msg", context.getString(R.string.empty_route))
            setResult(RESULT_CANCELED, data)
            finish()
            return
        }
    ...
    private fun skip() {
        ...
        if (rpc.routeProcessStatusId == RouteProcessStatus.processed.id) {
            makeText(
                binding.root, context.getString(R.string.already_processed), SnackBarType.INFO
            )
            return
        }
    ...
    private fun skipAll() {
        ...
        try {
            ...
            alert.setTitle(context.getString(R.string.skip_remaining))
            alert.setMessage(context.getString(R.string.do_you_want_to_skip_the_remaining_steps_question))
    ...
    private fun updateStatus(
        ...
        if (!rpc.saveChanges()) {
            makeText(
                binding.root,
                context.getString(R.string.error_updating_registered_data),
                ERROR
            )
        }
        ...
        if (User.hasPermission(PermissionEntry.ReentryRouteProcessContent)) {
            ...
            if (rpc != null) {
                if (rpc.routeProcessStatusId != RouteProcessStatus.processed.id) {
                    makeText(
                        binding.root,
                        context.getString(R.string.not_processed),
                        SnackBarType.INFO
                    )
                } else {
                    ScannerManager.lockScanner(this, true)
                    try {
                        val alert = AlertDialog.Builder(this)
                        alert.setTitle(context.getString(R.string.enter_record_again))
                        alert.setMessage(context.getString(R.string.are_you_sure_to_enter_this_record_again_question))
                        alert.setNegativeButton(R.string.cancel, null)
            ...
            makeText(
                binding.root,
                context.getString(R.string.you_do_not_have_permission_to_enter_collected_data_again),
                ERROR
            )
        }
    }
    ...
    private fun currentData() {
        ...
        if (dcId == null || rpc.status == RouteProcessStatus.notProcessed || rpc.status == RouteProcessStatus.skipped) {
            makeText(binding.root, context.getString(R.string.not_processed), SnackBarType.INFO)
            return
        }
    ...
    private fun confirmLevel(askConfirm: Boolean) {
        ...
        if (isRouteFinished()) {
            if (askConfirm) {
                ...
                try {
                    val alert = AlertDialog.Builder(this)
                    alert.setTitle(context.getString(R.string.finished_route))
                    alert.setMessage(context.getString(R.string.the_route_is_complete_do_you_want_to_confirm_and_return_to_the_main_menu_question))
                    alert.setNegativeButton(R.string.cancel, null)
                    alert.setPositiveButton(R.string.accept) { dialog, _ ->
                        dialog.dismiss()
                        saveRouteProcess()
    ...
    private fun isBackPressed() {
        ...
        try {
            val alert = AlertDialog.Builder(this)
            alert.setTitle(context.getString(R.string.suspend_the_data_collection))
            alert.setMessage(context.getString(R.string.you_want_to_suspend_the_data_collection_process))
            alert.setNegativeButton(R.string.cancel, null)
            alert.setPositiveButton(R.string.accept) { _, _ ->
                closeKeyboard(this)
    ...
    private fun processFinish(dc: DataCollection) {
        ...
        if (result == null && rc.expression.isNullOrEmpty() && (rc.trueResult != 0 || rc.falseResult != 0)) {
            ...
            makeText(
                binding.root,
                context.getString(R.string.route_design_error),
                ERROR
            )
            return
        }
        ...
        if (result == null && rc.trueResult != DcrResult.cont.id || result == null && rc.falseResult != DcrResult.cont.id) {
            ...
            makeText(
                binding.root,
                context.getString(R.string.invalid_value_does_not_allow_to_continue),
                ERROR
            )
            return
        }
        ...
        if (rc.trueResult == DcrResult.noContinue.id && result != null && result == true || rc.falseResult == DcrResult.noContinue.id && result != null && result == false) {
            ...
            makeText(
                binding.root,
                context.getString(R.string.value_does_not_allow_to_continue),
                ERROR
            )
            return
        }
    ...
    private fun showProgressDialog(it: AdapterProgress) {
        showProgressDialog(
            title = context.getString(R.string.processing_route),
            msg = it.msg,
