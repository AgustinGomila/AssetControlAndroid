package com.example.assetControl.ui.adapters.review
...
import com.example.assetControl.databinding.AssetRowBinding
import com.example.assetControl.databinding.AssetRowExpandedBinding
...
import com.example.assetControl.ui.adapters.interfaces.Interfaces.AddPhotoRequiredListener
...
import com.example.assetControl.ui.adapters.interfaces.Interfaces.EditAssetRequiredListener
...
class ArcRecyclerAdapter(
    ...
    companion object {
        ...
        private fun setupColors() {
            ...
            layoutRevisedForeColor =
                getBestContrastColor(getColor(context.resources, R.color.status_revised, null))
            layoutAppearedForeColor =
                getBestContrastColor(getColor(context.resources, R.color.status_appeared, null))
            layoutNotInReviewForeColor =
                getBestContrastColor(getColor(context.resources, R.color.status_not_in_review, null))
            layoutExternalForeColor =
                getBestContrastColor(getColor(context.resources, R.color.status_external, null))
            layoutNewForeColor = getBestContrastColor(getColor(context.resources, R.color.status_new, null))
            layoutDefaultForeColor =
                getBestContrastColor(getColor(context.resources, R.color.status_default, null))
        ...
        addPhotoImageView.setOnTouchListener { _, event ->
            if (event.action == MotionEvent.ACTION_DOWN) {
                addPhotoRequiredListener?.onAddPhotoRequired(
                    ...
                    obs = "${context.getString(R.string.user)}: ${currentUser()?.name}"
                )
            }
            true
        }
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            adapter.run {
                ImageAdapter.getImages(
                    context = context, programData = ProgramData(
    ...
    private fun updateContent(
        ...
        uiEventListener?.onUiEventRequired(
            AdapterProgress(
                totalTask = total,
                completedTask = 0,
                msg = context.getString(R.string.processing_asset),
        ...
        currentList.forEachIndexed { index, content ->
            uiEventListener?.onUiEventRequired(
                AdapterProgress(
                    ...
                    msg = context.getString(R.string.please_wait),
        ...
        uiEventListener?.onUiEventRequired(
            AdapterProgress(
                totalTask = total,
                completedTask = total,
                msg = context.getString(R.string.finished),
    ...
    class SelectedViewHolder(val binding: AssetRowExpandedBinding) : ViewHolder(binding.root) {
        ...
        fun setStyle(content: AssetReviewContent) {
            ...
            val layoutRevised = getDrawable(context.resources, R.drawable.layout_thin_border_green, null)
            val layoutAppeared = getDrawable(context.resources, R.drawable.layout_thin_border_red, null)
            val layoutNotInReview = getDrawable(context.resources, R.drawable.layout_thin_border_marron, null)
            val layoutExternal = getDrawable(context.resources, R.drawable.layout_thin_border_blue, null)
            val layoutNew = getDrawable(context.resources, R.drawable.layout_thin_border_yellow, null)
