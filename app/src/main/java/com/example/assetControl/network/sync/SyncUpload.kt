...
import com.example.assetControl.data.webservice.dataCollection.DataCollectionContentObject
import com.example.assetControl.data.webservice.dataCollection.DataCollectionObject
import com.example.assetControl.data.webservice.dataCollection.DataCollectionWs
...
import com.example.assetControl.data.webservice.maintenance.AssetMaintenanceLogObject
import com.example.assetControl.data.webservice.maintenance.AssetMaintenanceObject
import com.example.assetControl.data.webservice.maintenance.AssetMaintenanceWs
import com.example.assetControl.data.webservice.movement.WarehouseMovementContentObject
import com.example.assetControl.data.webservice.movement.WarehouseMovementObject
import com.example.assetControl.data.webservice.movement.WarehouseMovementWs
import com.example.assetControl.data.webservice.review.AssetReviewContentObject
import com.example.assetControl.data.webservice.review.AssetReviewObject
import com.example.assetControl.data.webservice.review.AssetReviewWs
import com.example.assetControl.data.webservice.route.RouteProcessContentObject
import com.example.assetControl.data.webservice.route.RouteProcessObject
import com.example.assetControl.data.webservice.route.RouteProcessWs
...
class SyncUpload(
            ...
            if (it.status == ProgressStatus.finished) {
                ...
                if (userId == null) {
                    onSyncTaskProgress.invoke(
                        SyncProgress(
                            msg = context.resources.getString(R.string.invalid_user),
    ...
    private fun onSyncFinish(result: Boolean) {
        if (result) {
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.synchronization_finished),
    ...
    private suspend fun suspendFunction(): Boolean = withContext(Dispatchers.IO) {
        onSyncTaskProgress.invoke(
            SyncProgress(
                msg = context.getString(R.string.synchronization_starting),
    ...
    private fun assetReview() {
        ...
        try {
            if (!scope.isActive) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        msg = context.getString(R.string.canceling_asset_review_synchronization),
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.starting_asset_review_synchronization),
                    registryType = registryType,
            ...
            for ((currentTask, ar) in arAl.toTypedArray().withIndex()) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        ...
                        msg = context.getString(R.string.synchronizing_asset_reviews),
                        registryType = registryType,
                ...
                if (arId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_asset_reviews)
                    )
                }
            }
        } catch (ex: Exception) {
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_asset_reviews),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_asset_reviews)
                        else -> context.getString(R.string.asset_review_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
    ...
    private fun warehouseMovement() {
        ...
        try {
            if (!scope.isActive) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        msg = context.getString(R.string.canceling_movement_synchronization),
                        registryType = registryType,
                        progressStatus = ProgressStatus.canceled
                    )
                )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.starting_movement_synchronization),
                    registryType = registryType,
                    progressStatus = ProgressStatus.starting
                )
            )
            ...
            for ((currentTask, wm) in wmAl.toTypedArray().withIndex()) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        ...
                        msg = context.getString(R.string.synchronizing_movements),
                        registryType = registryType,
                        progressStatus = ProgressStatus.running
                    )
                )
                ...
                if (wmId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_movements)
                    )
                }
            }
        } catch (ex: Exception) {
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_movements),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_movements)
                        else -> context.getString(R.string.movement_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
                        error -> ProgressStatus.crashed
    ...
    private fun asset() {
        ...
        try {
            if (!scope.isActive) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        msg = context.getString(R.string.canceling_asset_synchronization),
                        registryType = registryType,
                        progressStatus = ProgressStatus.canceled
                    )
                )
            ...
            for ((currentTask, a) in aAl.toTypedArray().withIndex()) {
                ...
                if (a.id > 0) {
                    realAssetId = assetWs.assetCollectorModify(
                        userId,
                        com.example.assetControl.data.webservice.asset.AssetCollectorObject(a)
                    )
                } else {
                ...
                if (realAssetId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_assets)
                    )
                }
            }
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_assets),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_assets)
                        else -> context.getString(R.string.asset_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
                        error -> ProgressStatus.crashed
    ...
    private fun warehouseArea() {
        ...
        try {
            if (!scope.isActive) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        msg = context.getString(R.string.canceling_warehouse_area_synchronization),
                        registryType = registryType,
                        progressStatus = ProgressStatus.canceled
                    )
                )
            ...
            for ((currentTask, wa) in waAl.toTypedArray().withIndex()) {
                ...
                if (realWarehouseAreaId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_warehouse_areas)
                    )
                }
            }
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_warehouse_areas),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_warehouse_areas)
                        else -> context.getString(R.string.warehouse_area_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
                        error -> ProgressStatus.crashed
    ...
    private fun warehouse() {
        ...
        try {
            if (!scope.isActive) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        msg = context.getString(R.string.canceling_warehouse_synchronization),
                        registryType = registryType,
                        progressStatus = ProgressStatus.canceled
                    )
                )
            ...
            for ((currentTask, w) in wAl.toTypedArray().withIndex()) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        ...
                        msg = context.getString(R.string.synchronizing_warehouses),
                        registryType = registryType,
                        progressStatus = ProgressStatus.running
                    )
                )
                ...
                if (realWarehouseId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_warehouses)
                    )
                }
            }
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_warehouses),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_warehouses)
                        else -> context.getString(R.string.warehouse_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
                        error -> ProgressStatus.crashed
    ...
    private fun itemCategory() {
        ...
        try {
            if (!scope.isActive) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        msg = context.getString(R.string.canceling_category_synchronization),
                        registryType = registryType,
                        progressStatus = ProgressStatus.canceled
                    )
                )
            ...
            for ((currentTask, ic) in icAl.toTypedArray().withIndex()) {
                ...
                if (realItemCategoryId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_categories)
                    )
                }
            }
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_categories),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_categories)
                        else -> context.getString(R.string.category_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
                        error -> ProgressStatus.crashed
    ...
    private fun dataCollection() {
        ...
        try {
            if (!scope.isActive) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        msg = context.getString(R.string.canceling_data_collection_synchronization),
                        registryType = registryType,
                        progressStatus = ProgressStatus.canceled
                    )
                )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.starting_data_collection_synchronization),
                    registryType = registryType,
                    progressStatus = ProgressStatus.starting
                )
            )
            ...
            for ((currentTask, dc) in dcAl.toTypedArray().withIndex()) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        ...
                        msg = context.getString(R.string.synchronizing_data_collections),
                        registryType = registryType,
                        progressStatus = ProgressStatus.running
                    )
                )
                ...
                if (dcId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_data_collections)
                    )
                }
            }
        } catch (ex: Exception) {
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_data_collections),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_data_collections)
                        else -> context.getString(R.string.data_collection_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
                        error -> ProgressStatus.crashed
    ...
    private fun routeProcess() {
        ...
        try {
            if (!scope.isActive) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        msg = context.getString(R.string.canceling_route_process_synchronization),
                        registryType = registryType,
                        progressStatus = ProgressStatus.canceled
                    )
                )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.starting_route_process_synchronization),
                    registryType = registryType,
                    progressStatus = ProgressStatus.starting
                )
            )
            ...
            for ((currentTask, rp) in rpAl.toTypedArray().withIndex()) {
                onSyncTaskProgress.invoke(
                    SyncProgress(
                        ...
                        msg = context.getString(R.string.synchronizing_route_process),
                        registryType = registryType,
                        progressStatus = ProgressStatus.running
                    )
                )
                ...
                if (rpId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_route_process)
                    )
                }
            }
        } catch (ex: Exception) {
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_route_process),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_route_process)
                        else -> context.getString(R.string.route_process_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
                        error -> ProgressStatus.crashed
        ...
        try {
            ...
            for ((currentTask, am) in amAl.toTypedArray().withIndex()) {
                ...
                if (amId > 0) {
                    ...
                    Log.d(
                        tag,
                        context.getString(R.string.failed_to_synchronize_the_maintenance_types)
                    )
                }
            }
        } catch (ex: Exception) {
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = context.getString(R.string.failed_to_synchronize_the_maintenance_types),
                    registryType = registryType,
                    progressStatus = ProgressStatus.crashed
                )
            )
            ...
            onSyncTaskProgress.invoke(
                SyncProgress(
                    msg = when {
                        error -> context.getString(R.string.failed_to_synchronize_the_maintenance_types)
                        else -> context.getString(R.string.maintenance_type_synchronization_completed)
                    },
                    registryType = registryType,
                    progressStatus = when {
                        error -> ProgressStatus.crashed
