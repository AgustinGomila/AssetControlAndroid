...
class DownloadDb(
    ...
    private suspend fun preExecute() = withContext(Dispatchers.IO) {
        timeFileLocation = File(context.cacheDir.absolutePath + "/" + timeFilename)
        dbFileLocation = File(context.cacheDir.absolutePath + "/" + dbFileName)
    ...
    private fun launchDownload() {
        ...
        if (!isLogged() && pendingDelivery()) {
            onDownloadEvent.invoke(
                DownloadTask(
                    msg = context.getString(R.string.the_database_will_not_be_downloaded_because_there_is_data_pending_delivery),
                    fileType = null,
                    downloadStatus = DownloadStatus.CANCELED,
                )
            )
    ...
    private fun onSyncUploadPendingProgress(it: SyncProgress) {
        ...
        var registryDesc = context.getString(R.string.all_tasks)
    ...
    private fun downloadDbFile() {
        ...
        if (!hasNewVersion(oldDateTimeStr, currentDateTimeStr)) {
            val msg = context.getString(R.string.is_not_necessary_to_download_the_database)
    ...
    private fun prepareDataBase() {
        ...
        if (copyDataBase()) {
            ...
            onDownloadEvent.invoke(
                DownloadTask(
                    context.getString(R.string.error_when_copying_database),
                    FileType.DBFILE,
    ...
    private fun copyDataBase(): Boolean {
        Log.d(this::class.java.simpleName, context.getString(R.string.copying_database))

        if (dbFileLocation == null || !dbFileLocation!!.exists()) {
            Log.e(
                this::class.java.simpleName,
                context.getString(R.string.database_file_does_not_exist)
            )
            return false
        }
        ...
        val outFileName = context.getDatabasePath(AcDatabase.DATABASE_NAME).toString()

        Log.d(
            this::class.java.simpleName,
            "${context.getString(R.string.origin)}: ${dbFileLocation!!.absolutePath}"
        )
        Log.d(
            this::class.java.simpleName,
        ...
        try {
            if (timeDateStr.isNotEmpty()) {
                ...
                scope.launch {
                    onUiEvent(
                        SnackBarEventData(
                            context.getString(R.string.the_creation_date_of_the_db_on_the_server_is_invalid),
                            SnackBarType.ERROR
                        )
                    )
                }
