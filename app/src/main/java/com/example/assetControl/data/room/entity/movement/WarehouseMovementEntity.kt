...
@Entity(
    tableName = WarehouseMovementEntry.TABLE_NAME,
    indices = [
        Index(
            value = [WarehouseMovementEntry.WAREHOUSE_MOVEMENT_ID],
            name = "IDX_${WarehouseMovementEntry.TABLE_NAME}_${WarehouseMovementEntry.WAREHOUSE_MOVEMENT_ID}"
        ),
        Index(
            value = [WarehouseMovementEntry.USER_ID],
            name = "IDX_${WarehouseMovementEntry.TABLE_NAME}_${WarehouseMovementEntry.USER_ID}"
        ),
        Index(
            value = [WarehouseMovementEntry.ORIGIN_WAREHOUSE_AREA_ID],
            name = "IDX_${WarehouseMovementEntry.TABLE_NAME}_${WarehouseMovementEntry.ORIGIN_WAREHOUSE_AREA_ID}"
        ),
        Index(
            value = [WarehouseMovementEntry.ORIGIN_WAREHOUSE_ID],
            name = "IDX_${WarehouseMovementEntry.TABLE_NAME}_${WarehouseMovementEntry.ORIGIN_WAREHOUSE_ID}"
        ),
        Index(
            value = [WarehouseMovementEntry.DESTINATION_WAREHOUSE_AREA_ID],
            name = "IDX_${WarehouseMovementEntry.TABLE_NAME}_${WarehouseMovementEntry.DESTINATION_WAREHOUSE_AREA_ID}"
        ),
        Index(
            value = [WarehouseMovementEntry.DESTINATION_WAREHOUSE_ID],
            name = "IDX_${WarehouseMovementEntry.TABLE_NAME}_${WarehouseMovementEntry.DESTINATION_WAREHOUSE_ID}"
        )
    ]
)
data class WarehouseMovementEntity(
    @PrimaryKey(autoGenerate = true)
    @ColumnInfo(name = WarehouseMovementEntry.ID) var id: Long = 0L,
    @ColumnInfo(name = WarehouseMovementEntry.WAREHOUSE_MOVEMENT_ID) var warehouseMovementId: Long = 0L,
    @ColumnInfo(name = WarehouseMovementEntry.WAREHOUSE_MOVEMENT_DATE) var warehouseMovementDate: Date = Date(),
    @ColumnInfo(name = WarehouseMovementEntry.OBS) var obs: String? = null,
    @ColumnInfo(name = WarehouseMovementEntry.USER_ID) var userId: Long = 0L,
    @ColumnInfo(name = WarehouseMovementEntry.ORIGIN_WAREHOUSE_AREA_ID) var originWarehouseAreaId: Long = 0L,
    @ColumnInfo(name = WarehouseMovementEntry.ORIGIN_WAREHOUSE_ID) var originWarehouseId: Long = 0L,
    @ColumnInfo(name = WarehouseMovementEntry.TRANSFERRED_DATE) var transferredDate: Date? = null,
    @ColumnInfo(name = WarehouseMovementEntry.DESTINATION_WAREHOUSE_AREA_ID) var destinationWarehouseAreaId: Long = 0L,
    @ColumnInfo(name = WarehouseMovementEntry.DESTINATION_WAREHOUSE_ID) var destinationWarehouseId: Long = 0L,
    @ColumnInfo(name = WarehouseMovementEntry.COMPLETED) var completed: Int? = null,
) {
