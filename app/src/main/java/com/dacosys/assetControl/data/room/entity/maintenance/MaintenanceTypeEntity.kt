...
import com.dacosys.assetControl.data.room.dto.maintenance.MaintenanceTypeEntry

@Entity(
    tableName = MaintenanceTypeEntry.TABLE_NAME,
    indices = [
        Index(
            value = [MaintenanceTypeEntry.MAINTENANCE_TYPE_GROUP_ID],
            name = "IDX_${MaintenanceTypeEntry.TABLE_NAME}_${MaintenanceTypeEntry.MAINTENANCE_TYPE_GROUP_ID}"
        ),
        Index(
            value = [MaintenanceTypeEntry.DESCRIPTION],
            name = "IDX_${MaintenanceTypeEntry.TABLE_NAME}_${MaintenanceTypeEntry.DESCRIPTION}"
        )
    ]
)
data class MaintenanceTypeEntity(
    @PrimaryKey @ColumnInfo(name = MaintenanceTypeEntry.ID) val id: Long,
