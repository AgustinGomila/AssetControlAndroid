...
class SetCurrentSession(private var onSessionCreated: (Boolean) -> Unit = {}) {
    ...
    private suspend fun suspendFunction(): Boolean = withContext(Dispatchers.IO) {
        ...
        var appName = "Unknown"
        val info = pInfo.applicationInfo
        if (info != null) {
            appName =
                "${pm.getApplicationLabel(info)} ${context.getString(R.string.app_milestone)} ${pInfo.versionName}"
        }
    ...
    init {
        scope.launch {
            ...
            onSessionCreated.invoke(result)
        }
    }
}