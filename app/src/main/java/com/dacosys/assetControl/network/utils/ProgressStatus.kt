...
class ProgressStatus : Parcelable {
    ...
    companion object CREATOR : Parcelable.Creator<ProgressStatus> {
        ...
        var unknown =
            ProgressStatus(0, context.getString(R.string.unknown))
        var starting = ProgressStatus(
            1,
            context.getString(R.string.progress_status_starting)
        )
        var running = ProgressStatus(
            2,
            context.getString(R.string.progress_status_running)
        )
        var success = ProgressStatus(
            3,
            context.getString(R.string.progress_status_success)
        )
        var canceled = ProgressStatus(
            4,
            context.getString(R.string.progress_status_canceled)
        )
        var crashed = ProgressStatus(
            5,
            context.getString(R.string.progress_status_crashed)
        )
        var finished = ProgressStatus(
            6,
            context.getString(R.string.progress_status_finished)
        )
